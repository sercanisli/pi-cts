    ...
    <div class="col-12">
      <div class="card">
        ...
        <div class="data-table">
          ...
          >
            ...
            <Column field="description" header="Açıklama" style="width: 15%">
              <template #body="slotProps">
                ...
                >
                  {{ slotProps.data.description.slice(0, 20)
                  }}{{ slotProps.data.description.length > 20 ? "..." : "" }}
                </span>
                <div v-if="slotProps.data.description">
                  <i
                    @click.prevent="handleDescription(slotProps.data)"
            ...
            <Column field="branchId" header="Yetkili" style="width: 5%">
              <template #body="slotProps">
                <div v-if="slotProps.data.branchPersons && slotProps.data.branchPersons.length > 0">
                  ...
                  text
                  rounded
                  @click="authorizedPerson(slotProps.data)"
                />
                </div>
          ...
          <Dialog
            v-model:visible="dialogDescription"
            :style="{ width: '400px' }"
            header="Açıklama"
            :modal="true"
            class="p-fluid"
          ...
          <Dialog
            v-model:visible="dialogAuthorizedPerson"
            :style="{ width: '400px' }"
            header="Yetkili Kişiler"
            :modal="true"
            class="p-fluid"
          >
      ...
      getAllConnectionsByBranchId().then((data) => {
        ...
        this.connections.forEach((detail) => {
          ...
          this.companies.forEach((company) => {
            if (
              company.companyId === detail.branchCompanyId &&
              company.companyName === detail.branchCompanyCompanyName
            ) {
          ...
          this.branches.forEach((branch) => {
            if (
              branch.branchId === detail.branchId &&
              branch.branchName === detail.branchBranchName &&
              branch.companyId === detail.branchCompanyId
            ) {
          ...
          this.connectionTypes.forEach((cType) => {
            if (
              cType.connectionTypeId === detail.connectionTypeId &&
              cType.connectionTypeType === detail.connectionTypeType
            ) {
          ...
          detail.branchPersons.forEach((person) => {
            ...
            this.persons.forEach((per) => {
              if (
                per.fullName === person.fullName &&
                per.phone === person.phone &&
                per.branchId === person.branchId
              ) {
